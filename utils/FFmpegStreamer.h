...
public:
    FFmpegStreamer(AVCodecID eCodecId, int nWidth, int nHeight, int nFps, const char *szInFilePath, const char *format = "rawvideo") : nFps(nFps) {
        ...
        if (avio_open(&oc->pb, oc->url, AVIO_FLAG_WRITE) < 0) {
            std::cerr << "FFMPEG: Could not open " << oc->url;
            return ;
        }
